...
- **Runners** (`band-runners.jpg`), between the five statuses and the erasure beat. One runner ahead of a chase group — which is what `ahead` and `behind` mean, shown straight after the section that defines them. Added because the first draft wrote runners out of the page entirely, which was wrong: `writePolicyFor` in `lib/geo.ts` has a tier at 2–6 m/s (7.2–21.6 km/h) that the 2026-09-04 design spec labels "running, easy riding", and that spec's opening sentence names running first among the things Radar is used for. The hero's audience list and the write-cadence line were corrected at the same time — the latter described a two-tier policy when it has three, and the tier it omitted was the runners'.
